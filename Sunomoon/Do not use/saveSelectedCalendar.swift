...
struct AttendeesStatusList1 {
    var email: String?
    var status: Int?
}

class saveSelectedCalenar: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    var showURLCalendarCell: Bool = true
    ...
    @IBAction func pressDelBtn(_ sender: Any) {
        performSegue(withIdentifier: "selectedDelCalendarSegue", sender: self)
    }
    
    // MARK: - Segue
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "selectedDelCalendarSegue"){
            ...
            if(indexPath.row == (rowInSection1 - 1)) {
                if(showNoteCalendarCell){
                    if(textViewHeight > 50){
                        height = textViewHeight + cellConstant
                    }
                }
            }
        }
    ...
    func getUITableViewCell(indentifier: String) -> UITableViewCell{
        ...
        case "LocationCalendarCell":
            cell = tableView.dequeueReusableCell(withIdentifier: indentifier) as! LocationCalendarCell
        ...
        case "NoteCalendarCell":
            ...
            textViewHeight = cell.textView.bounds.height
            if(textViewHeight  > 50){
                tableViewHeight = tableViewHeight - 60 + textViewHeight + cellConstant
                updateViewConstraints()
            }
            return cell
        default:
